...
def _normalize_akkadian(raw) -> str:
    ...
    return normalize_greek_token(raw)


def _is_junk_greek(s: str) -> bool:
    if not s:
        return True
    if len(s) <= 1:
        return True
...
def classify_anchor(anchor: dict, ctx: AuditContext) -> str:
    ...
    greek_raw = _normalize_akkadian(anchor.get("greek"))
    english_raw = str(anchor.get("english") or "").strip()
    confidence = float(anchor.get("confidence") or 0.0)

    if _is_junk_greek(greek_raw):
        return "junk_greek"
    if greek_raw in ctx.collision_keys:
        return "duplicate_collision"
    if confidence < ctx.low_conf_threshold:
        return "low_confidence"
    if greek_raw not in ctx.fused_vocab:
        return "greek_vocab_miss"
...
def classify_all(anchors: Iterable[dict], ctx: AuditContext) -> dict:
    ...
    low-confidence, greek_vocab_miss, or multiword_english buckets do NOT
...
def _recoverability_narrative(result: dict) -> str:
    ...
    high = buckets["greek_vocab_miss"]["count"]
    med = (
