"""
Production Export: Dual-view Greek alignment.

Projects the fused Greek vectors into BOTH whitened-EmbeddingGemma 768d
...
def project_all_vectors(
    ...
    """Project all Greek vectors into a target space using learned Ridge weights.
...
def main():
    ...
    print(f"Greek vectors: {akk_vectors.shape}, vocab: {len(akk_vocab)}")
    ...
    np.savez_compressed(
        str(FINAL_OUTPUT / "greek_aligned_vectors.npz"),
    ...
    np.savez_compressed(
        str(FINAL_OUTPUT / "greek_aligned_gemma_vectors.npz"),
        vectors=aligned_gemma,
    )
    print(f"Gemma aligned: {aligned_gemma.shape} ({aligned_gemma.dtype})")

    with open(FINAL_OUTPUT / "greek_aligned_vocab.json", "w", encoding="utf-8") as f:
    ...
    metadata = {
        ...
        "methodology": (
            "Cuneiformy-Greek dual-view "
            "(Greek 1536d -> whitened-EmbeddingGemma 768d primary, GloVe 300d secondary)"
        ),
        "shared": {
