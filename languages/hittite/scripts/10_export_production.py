"""
Production Export: Dual-view Hittite alignment.

Projects the fused Hittite vectors into BOTH whitened-EmbeddingGemma 768d
...
def project_all_vectors(
    hit_vectors: np.ndarray,
    coef: np.ndarray,
    intercept: np.ndarray,
) -> np.ndarray:
    """Project all Hittite vectors into a target space using learned Ridge weights.

    Works for any (target_dim, fused_dim) ridge. Returns fp16 for on-disk compactness.
    """
    projected = hit_vectors @ coef.T + intercept
...
def main():
    ...
    hit_vectors = fused_data["vectors"]
    hit_vocab = list(fused_data["vocab"])
    print(f"Hittite vectors: {hit_vectors.shape}, vocab: {len(hit_vocab)}")
    ...
    aligned_glove = project_all_vectors(hit_vectors, glove_coef, glove_intercept)
    np.savez_compressed(
        str(FINAL_OUTPUT / "hittite_aligned_vectors.npz"),
    ...
    aligned_gemma = project_all_vectors(hit_vectors, gemma_coef, gemma_intercept)
    np.savez_compressed(
        str(FINAL_OUTPUT / "hittite_aligned_gemma_vectors.npz"),
        vectors=aligned_gemma,
    )
    print(f"Gemma aligned: {aligned_gemma.shape} ({aligned_gemma.dtype})")

    with open(FINAL_OUTPUT / "hittite_aligned_vocab.json", "w", encoding="utf-8") as f:
        json.dump(hit_vocab, f, ensure_ascii=False)
    ...
    metadata = {
        ...
        "methodology": (
            "Cuneiformy-Hittite dual-view "
            "(Hittite 1536d -> whitened-EmbeddingGemma 768d primary, GloVe 300d secondary)"
        ),
        "shared": {
            "vocab_size": len(hit_vocab),
            "hittite_fused_dim": int(hit_vectors.shape[1]),
