...
Takes the hittite_vocab_miss anchors from the Workstream 2a audit and:
...
# hittite_normalize is always available; audit_anchors is imported lazily inside
...
def _load_lemma_surface_map(path: Path) -> dict[str, frozenset[str]]:
    ...
    for lemma in lemmas:
        ...
        if not cf_raw or not form_raw:
            continue
        cf = normalize_hittite_token(cf_raw)
        form = normalize_hittite_token(form_raw)
...
def _morphemes(anchor_raw: str) -> list[str]:
    ...
    for piece in anchor_raw.split("-"):
        # normalize_hittite_token handles subscripts, braces, ORACC->ATF, and lowercase.
        # Hyphens are already split out, so the hyphen-drop step is a no-op per piece.
        piece = normalize_hittite_token(piece)
        if piece:
            parts.append(piece)
    return parts
...
def classify_miss(
    ...
    hittite_raw = str(anchor.get("hittite") or "").strip()
    english = str(anchor.get("english") or "").lower()

    # 1. normalization_recoverable
    normalized = normalize_hittite_token(hittite_raw)
    ...
    for candidate in (hittite_raw, normalized):
        if not candidate:
            continue
    ...
    morphemes = _morphemes(hittite_raw)
    ...
    if hittite_raw and hittite_raw.isupper():
...
def simulate_ascii_normalize(misses: list[dict], ctx: "DiagnosticContext") -> dict:
    ...
    resolvable = 0
    for anchor in misses:
        normalized = normalize_hittite_token(str(anchor.get("hittite") or ""))
        if normalized and normalized in ctx.fused_vocab:
            resolvable += 1
    return {
...
def simulate_lower_min_count(misses: list[dict], ctx: "DiagnosticContext") -> dict:
    ...
    for threshold in (1, 2, 3, 4):
        ...
        for anchor in misses:
            ...
            if hittite_raw in ctx.fused_vocab or normalized in ctx.fused_vocab:
                continue
            for candidate in (hittite_raw, normalized):
                if not candidate:
                    continue
...
def render_markdown(
    ...
    lines.append(f"| hittite_vocab_miss (this diagnostic's input) | {baseline['hittite_vocab_miss']:,} |")
    lines.append("")
...
def run_diagnostic(
    *,
    ...
    from languages.hittite.scripts.audit_anchors import (  # noqa: PLC0415
        _load_anchors,
        _load_fused_vocab,
        _load_gemma_vocab,
        _load_glove_vocab,
    ...
    # The hittite_vocab_miss bucket is our input set.
    from languages.hittite.scripts.audit_anchors import AuditContext, classify_all
    ...
    misses = audit_result["buckets"]["hittite_vocab_miss"]["rows"]
    baseline = {
        "total_merged": audit_result["totals"]["merged"],
        "survives": audit_result["totals"]["survives"],
        "hittite_vocab_miss": audit_result["buckets"]["hittite_vocab_miss"]["count"],
    }
