"""
Production Export: Dual-view Akkadian alignment.

Projects the fused Akkadian vectors into BOTH whitened-EmbeddingGemma 768d
...
def project_all_vectors(
    ...
    """Project all Akkadian vectors into a target space using learned Ridge weights.
...
def main():
    ...
    print(f"Akkadian vectors: {akk_vectors.shape}, vocab: {len(akk_vocab)}")
    ...
    np.savez_compressed(
        str(FINAL_OUTPUT / "akkadian_aligned_vectors.npz"),
    ...
    np.savez_compressed(
        str(FINAL_OUTPUT / "akkadian_aligned_gemma_vectors.npz"),
    ...
    metadata = {
        ...
        "methodology": (
            "Cuneiformy-Akkadian dual-view "
            "(Akkadian 1536d -> whitened-EmbeddingGemma 768d primary, GloVe 300d secondary)"
        ),
        "shared": {
            "vocab_size": len(akk_vocab),
            "akkadian_fused_dim": int(akk_vectors.shape[1]),
