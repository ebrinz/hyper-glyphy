...
def _normalize_akkadian(raw) -> str:
    ...
    return normalize_akkadian_token(raw)


def _is_junk_akkadian(s: str) -> bool:
    if not s:
        return True
    if len(s) <= 1:
        return True
...
def classify_anchor(anchor: dict, ctx: AuditContext) -> str:
    ...
    akkadian_raw = _normalize_akkadian(anchor.get("akkadian"))
    english_raw = str(anchor.get("english") or "").strip()
    confidence = float(anchor.get("confidence") or 0.0)

    if _is_junk_akkadian(akkadian_raw):
        return "junk_akkadian"
    if akkadian_raw in ctx.collision_keys:
        return "duplicate_collision"
    if confidence < ctx.low_conf_threshold:
        return "low_confidence"
    if akkadian_raw not in ctx.fused_vocab:
        return "akkadian_vocab_miss"
...
def classify_all(anchors: Iterable[dict], ctx: AuditContext) -> dict:
    ...
    low-confidence, akkadian_vocab_miss, or multiword_english buckets do NOT
...
RECOVERABILITY = {
    "junk_akkadian":       "low — upstream extraction bug",
    "duplicate_collision": "low — dedup by design",
    "low_confidence":      "medium — raise threshold only if false-positive rate checks out",
    "akkadian_vocab_miss": "high — candidate driver for Workstream 2b (FastText retrain)",
...
def _recoverability_narrative(result: dict) -> str:
    ...
    high = buckets["akkadian_vocab_miss"]["count"]
    med = (
